...
const UI: Record<Language, Record<string, string>> = {
  en: {
    ...
    packFound: 'PACK WORD DISCOVERED', clickExplore: 'click to select', holdMove: 'hold to pick up', flickReroll: 'flick to reroll',
  ...
  he: {
    ...
    packFound: 'מילה מהחבילה התגלתה', clickExplore: 'לחצו לבחירה', holdMove: 'החזיקו כדי להרים', flickReroll: 'העיפו כדי להטיל שוב',
    possible: 'המשך אפשרי', complete: 'המילה הושלמה. אפשר לגלות אותה או להמשיך לחקור.',
    deadEnd: 'המסלול הזה לא מוביל למילה נסתרת זמינה.', noRerolls: 'לא נשארו הטלות חוזרות בסיבוב הזה.',
    selectFirst: 'בחרו קובייה אחת או יותר.', thrown: 'הקובייה הוטלה שוב.', moved: 'הקובייה הוזזה בלי לשנות את הפאה.',
    fresh: 'הטלה מתוכננת חדשה מהחבילה הנסתרת.',
  },
}
...
function freshMotion(die: DieState): Motion {
  return {
    ...
    targetQuaternion: landingQuaternion(die.targetFace), targetFace: die.targetFace, lastRollKey: die.rollKey,
    groundedTime: 0, settling: false, landedReported: false, held: false,
  }
}
...
function canFinishWord(word: string, prefix: string, letters: string[]): boolean {
  return word.startsWith(prefix) && canBuildWord(word.slice(prefix.length), letters)
}
...
function evaluateVisibleLetters(language: Language, letters: string[]): number {
  ...
  return starterDice * 8 + distinctStarters * 7 + completable * 14 - deadDice * 4 - minimumStarterPenalty - noWordPenalty
}
...
function chooseCuratedRerollFace(language: Language, dice: DieState[], dieId: number): number {
  ...
  const candidates = die.faces.map((letter, face) => {
    ...
    return { face, score: evaluateVisibleLetters(language, letters) + (letter === die.letter ? -8 : 0) + Math.random() * 4 }
  }).sort((a, b) => b.score - a.score)
...
function LooseDice({ dice, selectedIds, viableIds, onSelectNow, onDeselectClick, onLanded, onThrow, onPlaced }: {
  dice: DieState[]
  selectedIds: number[]
  viableIds: number[]
  onSelectNow: (id: number) => void
  onDeselectClick: (id: number) => void
  ...
  useEffect(() => () => {
    if (drag.current?.pickupTimer) clearTimeout(drag.current.pickupTimer)
  }, [])
      ...
      if (motion.held) {
        ...
        if (motion.position.y < floor) {
          ...
          if (motion.groundedTime > 0.22 && Math.abs(motion.velocity.y) < 0.22 && Math.hypot(motion.velocity.x, motion.velocity.z) < 0.42) {
            motion.settling = true; motion.velocity.set(0, 0, 0); motion.angularVelocity.set(0, 0, 0)
          }
        ...
        if (!motion.landedReported && motion.quaternion.angleTo(motion.targetQuaternion) < 0.025) {
          motion.quaternion.copy(motion.targetQuaternion); motion.landedReported = true; onLanded(die.id, die.faces[motion.targetFace])
        }
      }
    }
    for (const die of active) {
      ...
      if (group && motion) { group.position.copy(motion.position); group.quaternion.copy(motion.quaternion) }
    }
  })
  ...
  const pickUp = (id: number) => {
    ...
    if (motion) {
      motion.held = true; motion.settling = true; motion.velocity.set(0, 0, 0); motion.angularVelocity.set(0, 0, 0)
    }
  }
  ...
  const beginDrag = (event: ThreeEvent<PointerEvent>, die: DieState, selected: boolean) => {
    ...
    drag.current = {
      id: die.id, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY,
      lastX: event.clientX, lastY: event.clientY, lastTime: now, velocityX: 0, velocityY: 0,
      totalDistance: 0, startedAt: now, wasSelected: selected, pickedUp: false, pickupTimer: null,
    }
    drag.current.pickupTimer = setTimeout(() => pickUp(die.id), PICKUP_DELAY)
  ...
  const moveDrag = (event: ThreeEvent<PointerEvent>, die: DieState) => {
    ...
    current.velocityX = dx / dt; current.velocityY = dy / dt; current.totalDistance += Math.hypot(dx, dy)
    current.lastX = event.clientX; current.lastY = event.clientY; current.lastTime = now
  ...
  const endDrag = (event: ThreeEvent<PointerEvent>, die: DieState) => {
    ...
    const isClick = !current.pickedUp && current.totalDistance < CLICK_DISTANCE
    const isThrow = current.pickedUp && releaseSpeed >= THROW_SPEED && current.totalDistance >= CLICK_DISTANCE
    ...
    if (motion) {
      motion.held = false; motion.position.y = FLOOR_Y + HALF; motion.settling = true
      motion.velocity.set(0, 0, 0); motion.angularVelocity.set(0, 0, 0)
    }
    onPlaced()
  }
  ...
  return <>{dice.map(die => {
    ...
    return <group key={die.id} ref={node => { refs.current[die.id] = node }}
      onPointerDown={event => beginDrag(event, die, selected)}
      ...
      onPointerCancel={() => {
        ...
        if (current?.pickupTimer) clearTimeout(current.pickupTimer)
        const motion = motions.current[die.id]
        if (motion) { motion.held = false; motion.position.y = FLOOR_Y + HALF }
        drag.current = null
      }}>
      <RoundedBox args={[DIE_SIZE, DIE_SIZE, DIE_SIZE]} radius={0.035} smoothness={2} castShadow receiveShadow>
        <meshStandardMaterial
          color={selected ? '#f0c879' : viable ? '#d5dfcf' : '#d8cfc0'}
          emissive={selected ? '#79521e' : viable ? '#3e6a49' : '#000000'}
          emissiveIntensity={selected ? 0.16 : viable ? 0.18 : 0}
          roughness={0.88}
          metalness={0}
        />
      </RoundedBox>
...
function TrayGeometry() {
  return <group>
    <RoundedBox args={[6.65, 0.38, 4.25]} radius={0.26} smoothness={4} position={[0, FLOOR_Y - 0.18, 0]} receiveShadow><meshStandardMaterial color="#121820" roughness={0.94} /></RoundedBox>
...
function DiceTray(props: Parameters<typeof LooseDice>[0]) {
  return <Canvas shadows camera={{ position: [0, 9.4, 1.15], fov: 34 }} onCreated={({ camera }) => camera.lookAt(0, -0.35, 0)}>
    <color attach="background" args={['#11161d']} /><ambientLight intensity={1.15} />
    <directionalLight position={[4, 8, 4]} intensity={3.1} castShadow /><pointLight position={[-3.5, 1.6, -1.5]} color="#846ff0" intensity={6.5} distance={8} />
    <TrayGeometry /><LooseDice {...props} /><Environment preset="warehouse" />
  </Canvas>
}
...
export default function App() {
  ...
  const switchDiceLanguage = (language: Language) => {
    ...
    setDiceLanguage(language); setDice(createDice(language, Date.now())); setSelectedIds([]); setRerolls(3); setRound(value => value + 1); setSelectedCard(null); setMessage(UI[gameLanguage].fresh)
  }
  ...
  const throwDie = (id: number): boolean => {
    if (!rerolls) { setMessage(t.noRerolls); return false }
    const targetFace = chooseCuratedRerollFace(diceLanguage, dice, id)
    setDice(items => items.map(die => die.id === id ? { ...die, targetFace, letter: die.faces[targetFace], rollKey: die.rollKey + Date.now() } : die))
    setSelectedIds(ids => ids.filter(item => item !== id)); setRerolls(value => value - 1); setMessage(t.thrown); return true
  }
  const rerollSelected = () => {
    ...
    setSelectedIds([]); setRerolls(value => value - 1)
  }
  ...
  const submit = () => {
    ...
    setCards(deck => [...deck, { cardId: Date.now(), conceptId: entry.id, diceLanguage }]); setScore(value => value + currentWord.length * 10); setRevealId(entry.id); setSelectedIds([])
  }
  const matchConcept = (conceptId: string) => {
    ...
    setCards(deck => deck.filter(item => item.cardId !== card.cardId)); setSelectedCard(null); setScore(value => value + 50)
  }
  ...
  return <main className="game-shell" dir={gameLanguage === 'he' ? 'rtl' : 'ltr'}>
    ...
    <header className="topbar">
      <div className="stat"><span>{t.round}</span><strong>{round}</strong></div><div className="stat"><span>{t.score}</span><strong>{score.toLocaleString()}</strong></div>
      <div className="word-display"><span>{PACK.title[gameLanguage].toUpperCase()}</span><strong dir={direction}>{currentWord || '—'}</strong><small>{valid ? t.discoverable : pathMessage.toUpperCase()}</small></div>
