...
type WordEntry = { id:string; spelling:string; pointed:string; meaning:string; image:string; transliteration:string; packId:string }
type WordPack = { id:string; name:string; description:string; entries:WordEntry[] }
...
function landingQuaternion(face:number): Quaternion {
  ...
  return new Quaternion().setFromAxisAngle(new Vector3(0,1,0), uprightYaw).multiply(new Quaternion().setFromEuler(faceRotation))
}
...
function freshMotion(rollKey:number): Motion {
  ...
  return {
    ...
    targetQuaternion:landingQuaternion(targetFace), targetFace, lastRollKey:rollKey,
    groundedTime:0, settling:false, landedReported:false,
  }
}
  ...
  useFrame((_, rawDelta) => {
    ...
    for (const die of active) {
      ...
      if (!motion.settling) {
        ...
        if (motion.position.x < LEFT + HALF) { motion.position.x = LEFT + HALF; motion.velocity.x = Math.abs(motion.velocity.x) * 0.45; motion.angularVelocity.z += randomBetween(-2,2) }
        if (motion.position.x > RIGHT - HALF) { motion.position.x = RIGHT - HALF; motion.velocity.x = -Math.abs(motion.velocity.x) * 0.45; motion.angularVelocity.z += randomBetween(-2,2) }
        if (motion.position.z < BACK + HALF) { motion.position.z = BACK + HALF; motion.velocity.z = Math.abs(motion.velocity.z) * 0.45; motion.angularVelocity.x += randomBetween(-2,2) }
        if (motion.position.z > FRONT - HALF) { motion.position.z = FRONT - HALF; motion.velocity.z = -Math.abs(motion.velocity.z) * 0.45; motion.angularVelocity.x += randomBetween(-2,2) }
      } else {
...
function DiceTray({ dice, selectedIds, viableIds, onSelect, onLanded, onDragReroll }:{
  dice:DieState[]
  selectedIds:number[]
  viableIds:number[]
  onSelect:(id:number)=>void
  ...
  return <Canvas shadows camera={{ position:[0,9.4,1.15], fov:34, near:0.1, far:100 }} onCreated={({camera}) => camera.lookAt(0,-0.35,0)}>
    ...
    <TrayGeometry />
    <LooseDice dice={dice} selectedIds={selectedIds} viableIds={viableIds} onSelect={onSelect} onLanded={onLanded} onDragReroll={onDragReroll} />
...
export default function App() {
  ...
  const [message,setMessage] = useState('Click a die to select it. Drag a die to throw it again.')
  ...
  const viableIds = useMemo(() => {
    ...
    return dice.filter(die => !die.consumed && !selected.has(die.id)).filter(die => packSpellings.some(spelling => spelling.startsWith(currentWord + die.letter))).map(die => die.id)
  }, [currentWord,dice,selectedIds])

  const pathMessage = selectedIds.length===0 ? 'Click to select · drag to reroll' : viableIds.length>0 ? `${viableIds.length} guided ${viableIds.length===1?'path':'paths'} remain` : valid ? 'Word complete; discover it or keep exploring' : 'No pack path remains; free discovery is still allowed'
  ...
  const submit = () => {
    ...
    setReveal(entry)
    setMessage(entry.packId===ACTIVE_PACK.id ? `${currentWord} discovered in ${ACTIVE_PACK.name}.` : `${currentWord} found as a bonus discovery outside the guided pack.`)
    setSelectedIds([])
  }
  ...
  return <main className="game-shell">
    ...
    <section className={`repository panel ${repoOpen?'open':'closed'}`}>
      ...
      <div className="panel-content"><h2>DISCOVERED WORDS</h2><p>{ACTIVE_PACK.description}</p><div className="stack-list">
        {Object.entries(repository).length===0 && <div className="empty">Follow the glowing paths and discover a word to reveal its meaning.</div>}
    ...
    {reveal && <div className="discovery-overlay" role="dialog" aria-modal="true" aria-labelledby="discovery-title"><div className="discovery-card"><span className="reveal-kicker">{reveal.packId===ACTIVE_PACK.id?'WORD DISCOVERED':'BONUS DISCOVERY'}</span><span className="reveal-image" aria-hidden="true">{reveal.image}</span><h1 id="discovery-title" dir="rtl">{reveal.pointed}</h1><strong>{reveal.meaning}</strong><small>{reveal.transliteration}</small><p>{reveal.packId===ACTIVE_PACK.id?`Added to ${ACTIVE_PACK.name}.`:'Found beyond the guided paths.'}</p><button onClick={() => setReveal(null)}>CONTINUE</button></div></div>}
