...
import { FREE_WORDS, createVocabulary, getFreeWordInfo } from './data/freeVocabulary'
...
function chooseCuratedFaces(language: Language, mode: GameMode, words: string[], activeIds: number[] = PACK.diceFaces[language].map((_, id) => id)): number[] {
  ...
  const minimumWords = mode === 'free' ? Math.min(10, Math.max(4, Math.floor(usableWords.length / 25))) : Math.min(3, usableWords.length)
...
function chooseCuratedRerollFace(language: Language, mode: GameMode, words: string[], dice: DieState[], dieId: number): number {
  ...
  const candidates = die.faces.map((letter, face) => {
    ...
    const metrics = scoreRoll(words.filter(word => word.length <= letters.length), letters)
...
export default function App() {
  ...
  const candidateWords = useMemo(() => mode === 'pack' ? availableEntries.map(({ form }) => form.spelling) : FREE_WORDS[diceLanguage], [availableEntries, diceLanguage, mode])
  ...
  }, [candidateWords, currentWord, dice, selectedIds])

  const rollMetrics = useMemo(() => scoreRoll(candidateWords, dice.filter(die => !die.consumed).map(die => die.letter)), [candidateWords, dice])
  ...
  const switchMode = (next: GameMode) => { if (next === mode) return; const words = next === 'pack' ? activeForms.map(({ form }) => form.spelling) : FREE_WORDS[diceLanguage]; setMode(next); resetThrow(next, diceLanguage, words); setReward(null); setSelectedCard(null); setMessage(next === 'free' ? t.emptyFree : PACK.description[gameLanguage]) }
  const switchDiceLanguage = (language: Language) => { if (language === diceLanguage) return; const words = mode === 'pack' ? PACK.entries.map(item => item.forms[language].spelling) : FREE_WORDS[language]; setDiceLanguage(language); setDice(freshDice(language, mode, words)); setSelectedIds([]); setRerolls(3); setRound(value => value + 1); setSelectedCard(null); setReward(null); setMessage(UI[gameLanguage].fresh) }
  ...
  const throwDie = (id: number): boolean => { if (!rerolls) { setMessage(t.noRerolls); return false }; const targetFace = chooseCuratedRerollFace(diceLanguage, mode, candidateWords, dice, id); setDice(items => items.map(die => die.id === id ? { ...die, targetFace, letter: die.faces[targetFace], rollKey: die.rollKey + Date.now() } : die)); setSelectedIds(ids => ids.filter(item => item !== id)); setRerolls(value => value - 1); setMessage(t.thrown); return true }
  const rerollSelected = () => { if (!rerolls) { setMessage(t.noRerolls); return }; if (!selectedIds.length) { setMessage(t.selectFirst); return }; const ids = new Set(selectedIds); setDice(items => items.map(die => { if (!ids.has(die.id)) return die; const targetFace = chooseCuratedRerollFace(diceLanguage, mode, candidateWords, items, die.id); return { ...die, targetFace, letter: die.faces[targetFace], rollKey: die.rollKey + Date.now() } })); setSelectedIds([]); setRerolls(value => value - 1) }
  const shakeRemaining = () => { setDice(items => curateRemainingDice(items, diceLanguage, mode, candidateWords, Date.now())); setSelectedIds([]) }
