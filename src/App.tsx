...
const UI: Record<Language, Record<string, string>> = {
  en: {
    diceLanguage: 'Dice Language', gameLanguage: 'Game Language', round: 'ROUND', score: 'SCORE', rerolls: 'REROLLS', discoverable: 'DISCOVERABLE WORD', discoveredWords: 'DISCOVERED WORDS', hiddenWords: 'hidden words discovered', emptyRepo: 'The pack contains hidden household words. Follow a glowing path to discover the first one.', shake: 'SHAKE', rerollSelected: 'REROLL SELECTED', nextRound: 'NEXT ROUND', discover: 'DISCOVER', decipher: 'DECIPHER', emptyDeck: 'Discovered words create recall cards here.', match: 'Match to its spelling', continue: 'CONTINUE', packFound: 'PACK WORD DISCOVERED', clickExplore: 'click to select', holdMove: 'hold to pick up', flickReroll: 'flick to reroll', possible: 'possible continuation', complete: 'A word is complete. Discover it or continue freely.', deadEnd: 'This path does not reach an available hidden pack word.', noRerolls: 'No rerolls remain this round.', selectFirst: 'Select one or more dice first.', thrown: 'Die thrown again.', moved: 'Die moved without changing its face.', fresh: 'A fresh curated roll from the hidden pack.',
  },
  he: {
    diceLanguage: 'שפת הקוביות', gameLanguage: 'שפת המשחק', round: 'סיבוב', score: 'ניקוד', rerolls: 'הטלות חוזרות', discoverable: 'מילה ניתנת לגילוי', discoveredWords: 'מילים שהתגלו', hiddenWords: 'מילים נסתרות התגלו', emptyRepo: 'החבילה מכילה מילים נסתרות מהבית. עקבו אחרי מסלול זוהר כדי לגלות את הראשונה.', shake: 'נער', rerollSelected: 'הטל נבחרות', nextRound: 'סיבוב הבא', discover: 'גלה', decipher: 'פענוח', emptyDeck: 'מילים שהתגלו יוצרות כאן קלפי זיכרון.', match: 'התאם לאיות שלה', continue: 'המשך', packFound: 'מילה מהחבילה התגלתה', clickExplore: 'לחצו לבחירה', holdMove: 'החזיקו כדי להרים', flickReroll: 'העיפו כדי להטיל שוב', possible: 'המשך אפשרי', complete: 'המילה הושלמה. אפשר לגלות אותה או להמשיך לחקור.', deadEnd: 'המסלול הזה לא מוביל למילה נסתרת זמינה.', noRerolls: 'לא נשארו הטלות חוזרות בסיבוב הזה.', selectFirst: 'בחרו קובייה אחת או יותר.', thrown: 'הקובייה הוטלה שוב.', moved: 'הקובייה הוזזה בלי לשנות את הפאה.', fresh: 'הטלה מתוכננת חדשה מהחבילה הנסתרת.',
  },
}
    ...
    return <group key={die.id} ref={node => { refs.current[die.id] = node }} onPointerDown={event => beginDrag(event, die, selected)} onPointerMove={event => moveDrag(event, die)} onPointerUp={event => endDrag(event, die)} onPointerCancel={() => { const current = drag.current; if (current?.pickupTimer) clearTimeout(current.pickupTimer); const motion = motions.current[die.id]; if (motion) { motion.held = false; motion.position.y = FLOOR_Y + HALF }; drag.current = null }}>
      {viable && !selected && <>
        <mesh scale={1.22}>
          ...
          <meshBasicMaterial color="#69e69a" wireframe transparent opacity={0.9} depthTest={false} />
        </mesh>
        <mesh position={[0, -HALF - 0.018, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <ringGeometry args={[0.29, 0.43, 40]} />
          <meshBasicMaterial color="#65e894" transparent opacity={0.62} depthWrite={false} />
        </mesh>
        <pointLight position={[0, 0.2, 0]} color="#68efa0" intensity={3.2} distance={1.5} decay={2} />
      </>}
      <RoundedBox args={[DIE_SIZE, DIE_SIZE, DIE_SIZE]} radius={0.035} smoothness={2} castShadow receiveShadow>
        <meshStandardMaterial color={selected ? '#f0c879' : viable ? '#b8e4c2' : '#d8cfc0'} emissive={selected ? '#79521e' : viable ? '#24a653' : '#000000'} emissiveIntensity={selected ? 0.16 : viable ? 0.72 : 0} roughness={0.88} metalness={0} />
      </RoundedBox>
...
export default function App() {
  ...
  const viableIds = useMemo(() => {
    ...
    return dice.filter(die => !die.consumed && !selected.has(die.id)).filter(candidate => {
      ...
      return availableEntries.some(({ form }) => form.spelling.startsWith(prefix))
    }).map(die => die.id)
  }, [availableEntries, currentWord, dice, selectedIds])

  const pathMessage = !selectedIds.length ? `${Object.keys(currentDiscoveries).length}/${PACK.entries.length} ${t.hiddenWords} · ${t.clickExplore} · ${t.holdMove} · ${t.flickReroll}` : viableIds.length ? `${viableIds.length} ${t.possible}${viableIds.length === 1 ? '' : 's'}` : valid ? t.complete : t.deadEnd
  ...
  return <main className="game-shell" dir={gameLanguage === 'he' ? 'rtl' : 'ltr'}>
    ...
    <section className={`repository panel ${repoOpen ? 'open' : 'closed'}`}><button className="panel-tab" onClick={() => setRepoOpen(open => !open)} aria-label="Toggle discovered words">{repoOpen ? '‹' : '›'}</button><div className="panel-content"><h2>{t.discoveredWords}</h2><p>{Object.keys(currentDiscoveries).length} / {PACK.entries.length} · {PACK.description[gameLanguage]}</p><div className="stack-list">{!Object.keys(currentDiscoveries).length && <div className="empty">{t.emptyRepo}</div>}{PACK.entries.filter(item => currentDiscoveries[item.id]).map(item => { const form = item.forms[diceLanguage]; return <details key={item.id} className="word-discovery"><summary><b dir={direction}>{form.spelling}</b><span>×{currentDiscoveries[item.id]}</span></summary><button className="discovery-detail" onClick={() => matchConcept(item.id)}><span className="discovery-icon">{item.image}</span><span><strong dir={direction}>{form.display}</strong><small>{item.meanings[gameLanguage]} · {form.pronunciation}</small></span></button></details> })}</div></div></section>
